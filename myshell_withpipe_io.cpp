...
while(1)
{
   ...
   if(pipecount==0&&append==0&&overwrite==0)
        {processed_ip=ProcessString(in);cout<<"inside basic";}
   else if(pipecount>0)
        processed_ip=ProcessString_withPipe(in);
   else if(append>0)
        {processed_ip=ProcessString_withAppend(in);cout<<"inside append\n"<<append<<endl;
         cout<<"meenu"<<processed_ip[0]<<" "<<processed_ip[1]<<endl;
         //executeow(processed_ip);
         }
   else if(overwrite>0)
        {processed_ip=ProcessString_withOverwrite(in);cout<<"inside overwrite\n"<<overwrite<<endl;
         cout<<processed_ip[0]<<" "<<processed_ip[1]<<endl;
         //executeow(processed_ip);
         }
